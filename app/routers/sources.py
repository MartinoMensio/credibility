...
def get_sources(request_body: input_classes.BatchSourceRequest):
    ...
    try:
        ...
        raise HTTPException(
            422,
            {
                "message": "ConnectionError with some of the realtime APIs",
                "suggestion": "perform requests in smaller chunks, like 400",
            },
        )


@router.get(
    "/all", response_model=List[output_classes.AggregatedAssessment]
)  # response_model=List[output_classes.domain]
def get_all_sources():
