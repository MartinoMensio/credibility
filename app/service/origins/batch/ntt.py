...
class Origin(OriginBatch):
    def __init__(self):
        ...
        self.api_endpoint = (
            "https://www.newsroomtransparencytracker.com/wp-admin/admin-ajax.php"
        )
...
def _interpret_table(table, origin_id, homepage):
    ...
    for row in table["data"]:
        ...
        partial_compliant_cnt = sum(
            [1 for el in row_parsed.values() if el == "Partial"]
        )
...
def _get_credibility_measures(row):
    ...
    score = score * 2 - 1
    return {"value": score, "confidence": 1.0}
