...
class Origin(OriginBatch):
    ...
    def retreive_source_assessments(self):
        return _retrieve_assessments(self.id, self.homepage, self.spreadsheet_id, self.api_key)
...
def _get_table(spreadsheet_id, api_key, sheet_name):
    ...
    response = requests.get(f'https://sheets.googleapis.com/v4/spreadsheets/{spreadsheet_id}/values/{sheet_name}?key={api_key}')
