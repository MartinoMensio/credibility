...
class Origin(OriginRealtime):
    ...
    def retrieve_domain_credibility(self, domain):
        return _retrieve_assessment(
            domain, self.api_endpoint, self.api_key, self.api_userid, self.id
        )

    def retrieve_source_credibility(self, source):
        result = _retrieve_assessment(
            source, self.api_endpoint, self.api_key, self.api_userid, self.id
        )
    ...
    def retrieve_domain_credibility_multiple(self, domains):
        return _retrieve_assessment_multiple(
            domains, self.api_endpoint, self.api_endpoint, self.id
        )


def _retrieve_assessment(source, api_endpoint, api_key, api_userid, origin_id):
    api_response = _query_api([source], api_endpoint, api_key, api_userid).get(
        source, {}
    )
    return _pack_response(api_response, origin_id)


def _retrieve_assessment_multiple(
    sources, api_endpoint, api_key, api_userid, origin_id
):
...
def _query_api(hosts, api_endpoint, api_key, api_userid):
    ...
    if len(hosts) > 100:
        raise ValueError(
            f"API only allows up to 100 hosts per request! Received call with {len(hosts)}"
        )
    # hosts_param = '/'.join(hosts) + '/'
    response = requests.get(
        api_endpoint,
        params={"t": hosts},
        headers={"x-api-key": api_key, "x-user-id": api_userid},
    )
